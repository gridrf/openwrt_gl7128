...
char Base64::code_to_char(uint8_t x) 
{
	if (x <= 25) {
		return 'A' + x;
	}
		...
		printf("ERROR: %i IS OUT OF RANGE 0-63 FOR BASE64 ENCODING\n", x);
		return -1;
	} //TODO: improve error management
}

uint8_t Base64::char_to_code(char x) 
{
	if ((x >= 'A') && (x <= 'Z')) {
		...
		printf("ERROR: %c (0x%x) IS INVALID CHARACTER FOR BASE64 DECODING\n", x, x);
		return -1;
	} //TODO: improve error management
}

int Base64::bin_to_b64_nopad(const uint8_t * in, int size, char * out, int max_len)
{
	int i;
	...
	if (max_len < (result_len + 1)) { /* 1 char added for string terminator */
		printf("ERROR: OUTPUT BUFFER TOO SMALL IN BIN_TO_B64\n");
		return -1;
	}
...
int Base64::b64_to_bin_nopad(const char * in, int size, uint8_t * out, int max_len) 
{
	int i;
	int result_len; /* size of the result */
	int full_blocks; /* number of 3 unsigned chars / 4 characters blocks */
	...
	if (last_bytes == 1) {
		...
		out[3 * i + 0] = (b >> 16) & 0xFF;
		if (((b >> 12) & 0x0F) != 0) {
			printf("WARNING: last character contains unusable bits\n");
		}
	}
...
int Base64::bin_to_b64(const uint8_t * in, int size, char * out, int max_len) 
{
	int ret;
	...
	case 0: /* nothing to do */
		return ret;
	case 1:
		printf("ERROR: INVALID UNPADDED BASE64 STRING\n");
		return -1;
	case 2: /* 2 chars in last block, must add 2 padding char */
		if (max_len >= (ret + 2 + 1)) {
			...
			printf("ERROR: not enough room to add padding in bin_to_b64\n");
			return -1;
		}
...
int Base64::b64_to_bin(const char * in, int size, uint8_t * out, int max_len)
{
	if (in == NULL) {
		printf("ERROR: NULL POINTER AS OUTPUT OR INPUT IN B64_TO_BIN\n");
		return -1;
